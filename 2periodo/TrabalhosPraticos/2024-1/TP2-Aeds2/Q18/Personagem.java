 ...
 *  TP1-Q18: Ordenação por Quicksort Parcial em Java
 */
 ...
 class Personagem{
     ...
     public static void mostrar(Personagem personagens[]){
        for(int i=0;i<10 && personagens[i]!=null;i++){
    ...
    */
    public static void quicksortParcial(Personagem sub[], int esq, int dir, int k, Log log) {
        int i = esq, j = dir;
        Personagem pivo = sub[(dir+esq)/2];
        while (i <= j) {
            while (comparator(sub[i], pivo, log)) i++;
            while (comparator(pivo, sub[j], log)) j--;
            if (i <= j) {
                swap(sub, i, j, log);
                i++;
                j--;
        ...
        if (esq < j)  quicksortParcial(sub, esq, j, k, log);
        if (i < dir && i < k)  quicksortParcial(sub, i, dir, k, log);
    }
    ...
    public static void main(String[] args) throws Exception{
        ...
        quicksortParcial(personagens, 0, tam-1, 10, log);
