...
def bresenham(x1, y1, x2, y2):
    """
    Algoritmo de Bresenham para calcular os pontos de uma reta.
    Funciona para qualquer direção da reta.
    """
    ...
    # Define a direção do movimento em X
    if x1 < x2:
        sx = 1
    else:
        sx = -1

    # Define a direção do movimento em Y
    if y1 < y2:
        sy = 1
    else:
        sy = -1
    ...
    while True:
        ...
        # Verifica se deve avançar no eixo X
        if erro2 > -dy:
            erro -= dy
            x += sx

        # Verifica se deve avançar no eixo Y
        if erro2 < dx:
            erro += dx
            y += sy
...
def desenhar_reta():
    ...
    # Calcula os pontos utilizando Bresenham
    pontos = bresenham(x1, y1, x2, y2)

    # Desenha cada pixel da reta
    for x, y in pontos:
...
janela = tk.Tk()
janela.title("Algoritmo de Bresenham")
