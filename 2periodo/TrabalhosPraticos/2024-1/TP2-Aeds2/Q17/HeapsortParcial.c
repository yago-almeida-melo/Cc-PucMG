...
/*
    functions: heapsort, construir, reconstruir e getMaiorFilho
    @params: Personagem[] && ints
    action: Funcoes do algoritmo de heapsort, ordena por hairColour e desempate por nome
*/
void construir(Personagem tmp[], int tamHeap){
    for(int i = tamHeap; i > 1 && !comparator(tmp[i], tmp[i/2]); i /= 2){
        swap(&tmp[i], &tmp[i/2]);
    }
}

int getMaiorFilho(Personagem tmp[], int i, int tamHeap){
    int filho = 0;
    if (2*i == tamHeap || !comparator(tmp[2*i], tmp[2*i+1])){
        filho = 2*i;
    } else {
        filho = 2*i + 1;
    }
...
void reconstruir(Personagem tmp[], int tamHeap){
    int i = 1;
    while(i <= (tamHeap/2)){
        int filho = getMaiorFilho(tmp, i, tamHeap);
        if(comparator(tmp[i], tmp[filho])){
...
void heapsort(int key) {
    start = clock();
    Personagem tmp[tamSubArray+1];
    for(int i=0;i<tamSubArray;i++){
        tmp[i+1] = sub[i];
    }
    for(int tamHeap = 2; tamHeap <= key; tamHeap++){
        construir(tmp, tamHeap);
    }

    for(int i = key+1;i<=tamSubArray;i++){
        if(comparator(tmp[i], tmp[1])){
            swap(&tmp[i], &tmp[1]);
            reconstruir(tmp, key);
        }
    }
...
void registroLog(int key){
    FILE *arquivo;
    if(key == 0){
        arquivo = fopen("806454_heapsort.txt", "w");
    }else{
        arquivo = fopen("/tmp/806454_heapsort.txt", "w");
    }
    if (arquivo == NULL) {
        perror("Erro ao abrir o arquivo.");
        exit(EXIT_FAILURE); 
