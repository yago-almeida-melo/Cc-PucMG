 ...
 *  TP4-Q07: Hash Indireta com Lista Simples em Java
 */
...
class Hash {
    ...
    public int h(Personagem elemento) {
        return elemento.getYearOfBirth() % tamanho;
    }

    int pesquisar(Personagem elemento, Log log) {
        int pos = h(elemento);
        boolean x = tabela[pos].pesquisar(elemento);
        if(x) { return pos; } else{ return -1; }
    }
...
class ListaSimples{
    ...
    /**
     * Insere elemento em um indice especifico. Considera que primeiro elemento
     * esta no indice 0.
     *
...
class Personagem {
    ...
    public static Personagem procuraNome(Personagem personagens[], String nome) {
        Personagem resp = new Personagem();
        for (int i = 0; i < personagens.length; i++) {
    ...
    public static void inserirNaHash(Hash hash, Personagem personagens[], Scanner sc) throws Exception {
