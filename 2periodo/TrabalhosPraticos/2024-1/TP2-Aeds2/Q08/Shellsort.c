...
/*
    function: shellsort
    @params: null
    action: Ordena o sub-array usando o metodo de shellsort, pelo atributo eyeOfColour
*/
void shellsort(){
    start = clock();
    int h = 1;
    do { h = (h * 3) + 1; } while (h < tamSubArray);
    do {
        h /= 3;
...
void registroLog(int key){
    FILE *arquivo;
    if(key == 0){
        arquivo = fopen("806454_shellsort.txt", "w");
    }else{
        arquivo = fopen("/tmp/806454_shellsort.txt", "w");
    }
    if (arquivo == NULL) {
        perror("Erro ao abrir o arquivo.");
        exit(EXIT_FAILURE); 
