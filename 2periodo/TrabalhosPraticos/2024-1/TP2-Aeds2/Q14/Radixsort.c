...
/*
    function: comparator
    @params: Personagem && Personagem
    action: Compara dois Personagens pelo atributo id (desempate por nome) e retorna um booleano
*/
bool comparator(Personagem a, Personagem b){
    bool resp = false;  
    if(strcmp(a.id, b.id) < 0){       // a < b
        resp = true; 
        comparacoes++;  
    } else if(strcmp(a.id, b.id) > 0){  // a > b
        resp = false;
        comparacoes++;
    } else{                               // a==b   
...
void registroLog(int key){
    FILE *arquivo;
    if(key == 0){
        arquivo = fopen("806454_radixsort.txt", "w");
    }else{
        arquivo = fopen("/tmp/806454_radixsort.txt", "w");
    }
    if (arquivo == NULL) {
        perror("Erro ao abrir o arquivo.");
        exit(EXIT_FAILURE); 
