...
bool busca(No* no, char* str);
void registroLog(long tempo, int comparacoes);
...
bool busca(No* no, char* str) {
    ...
    int comparacao = strcmp(str, no->elemento.nome);
    if (comparacao < 0) {
        printf(" esq"); 
...
int main()
{
    printf("806454\tYago Almeida Melo\n\n");
    addJogadores();
    char str[100];
    scanf(" %[^\n]s", str);
