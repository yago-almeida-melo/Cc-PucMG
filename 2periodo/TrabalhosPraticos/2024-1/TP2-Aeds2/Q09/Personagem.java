 ...
 */
 ...
 class Personagem{
     ...
     * action: Compara dois Personagens pela cor de cabelo, desempate por nome
     */
    public static boolean comparator(Personagem d1, Personagem d2, Log log){
        boolean resp = false;
         if(d1.getHairColour().compareTo(d2.getHairColour()) > 0){   // resp = -1 -> d1 < d2
            resp = true;
            log.incrementaComp();
         } else if(d1.getHairColour().compareTo(d2.getHairColour()) < 0){   // resp = 1 -> d1 > d2 
            resp = false;
            log.incrementaComp();
         } else{
     ...
     /*
      * function: heapsort, construir, reconstruir e getMaiorFilho
      * @params: Personagem[] && int && Log 
      * action: Ordena o Array de Personagem pelo atributo dateOfBirth, usando Heapsort
      */
      public static void heapsort(Personagem array[], int n, Log log) {
        ...
        for(int tamHeap = 2; tamHeap <= n; tamHeap++){
           construir(array,tamHeap, log);
        }
        int tamHeap = n;
        while(tamHeap > 1){
           swap(array, 1, tamHeap--, log);
           reconstruir(array, tamHeap, log);
        }
        tmp = array;
     ...
     public static void construir(Personagem array[], int tamHeap, Log log){
        for(int i = tamHeap; i > 1 && comparator(array[i], array[i/2], log); i /= 2){
           swap(array, i, i/2, log);
        }
     }

     public static void reconstruir(Personagem array[], int tamHeap, Log log){
        int i = 1;
        while(i <= (tamHeap/2)){
           int filho = getMaiorFilho(array, i, tamHeap, log);
           if(!comparator(array[i], array[filho], log)){
     ...
     public static int getMaiorFilho(Personagem array[], int i, int tamHeap, Log log){
        int filho;
        if (2*i == tamHeap || comparator(array[2*i], array[2*i+1], log)){
           filho = 2*i;
        } else {
           filho = 2*i + 1;
        }
